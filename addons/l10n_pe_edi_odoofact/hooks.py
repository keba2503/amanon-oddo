...
def _deactivate_edi_enterprise(env):
    """Ejecutado después de instalar el módulo para configurar la integración con Enterprise"""

    # Verificar si l10n_pe_edi (módulo Enterprise) está instalado
    l10n_pe_edi_module = env["ir.module.module"].search(
        [("name", "=", "l10n_pe_edi"), ("state", "=", "installed")], limit=1
    )

    if l10n_pe_edi_module:
        # Estamos en Enterprise con el módulo l10n_pe_edi instalado
        # Desactivar la configuración de UBL solo para empresas peruanas
        companies = env["res.company"].search([('country_id.code', '=', 'PE')])
        
        for company in companies:
            # Desactivar UBL solo para empresas de Perú
            if hasattr(company, "l10n_pe_edi_provider"):
                company.l10n_pe_edi_provider = ""

            # Registrar mensaje en el log
            env["mail.message"].create(
                {
                    "body": "Módulo Nubefact activado como proveedor EDI oficial. Sistema UBL nativo desactivado para esta empresa peruana.",
                    "model": "res.company",
                    "res_id": company.id,
                    "message_type": "notification",
                    "subject": "Configuración EDI Perú",
                }
            )
...
def post_init_hook(env):
    ...
    _deactivate_edi_enterprise(env)
