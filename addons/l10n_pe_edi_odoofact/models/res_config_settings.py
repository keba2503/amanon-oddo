...
class ResConfigSettings(models.TransientModel):
    ...
    activate_einvoice_ticket = fields.Boolean(
        string="Print Electronic Ticket",
        related="company_id.activate_einvoice_ticket",
        readonly=False,
    )

    def action_open_company_edi_settings(self):
        """Abre la configuración de la compañía en la pestaña de EDI"""
        self.ensure_one()
        return {
            ...
            'view_mode': 'form',
            'view_id': self.env.ref('l10n_pe_edi_odoofact.view_company_form_inherit').id,
            'target': 'current',
            'context': {
                'form_view_initial_mode': 'edit',
                'default_page': 'peruvian_einvoice'  # Abre directamente en la pestaña de EDI
            }
        }
