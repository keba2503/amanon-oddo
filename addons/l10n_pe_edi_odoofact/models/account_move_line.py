    ...
    def _compute_l10n_pe_edi_amounts(self):
        ...
        for line in self:
            ...
            if line.display_type == "product" and line.tax_ids:
                ...
                line.l10n_pe_edi_price_unit_excluded = float_round(line.price_subtotal / line.quantity, precision_digits=price_precision) if line.quantity else 0.0
                line.l10n_pe_edi_price_unit_included = float_round(line.price_total / line.quantity, precision_digits=price_precision) if line.quantity else 0.0
                ...
                # La suma de los impuestos IGV suman CERO si es gratuito
                line.l10n_pe_edi_igv_amount = sum(
                    x["amount"]
                    for x in taxes_res["taxes"]
                    if x["id"] in igv_taxes_ids.ids
                ) if not line.l10n_pe_edi_free_product else 0.0
            ...
            if not line.l10n_pe_edi_free_product:
                ...
                # Para las ventas gratuitas el precio unitario es igual al valor unitario (Sin impuestos) 
                line.l10n_pe_edi_price_unit_included = line.l10n_pe_edi_price_unit_excluded

    @api.depends("tax_ids")
    def _compute_l10n_pe_edi_free_product(self):
        for line in self:
            line.l10n_pe_edi_free_product = any(tax.l10n_pe_edi_tax_code in ["9996"] for tax in line.tax_ids)
