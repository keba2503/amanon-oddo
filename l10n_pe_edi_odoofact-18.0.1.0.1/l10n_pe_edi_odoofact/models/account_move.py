...
class AccountMove(models.Model):
    ...
    @api.depends('company_id','company_id.activate_einvoice_ticket')
    def _compute_activate_einvoice_ticket(self):
        for rec in self:
            rec.activate_einvoice_ticket = rec.company_id.activate_einvoice_ticket
    ...
    l10n_pe_edi_odoofact_operation_type = fields.Selection(
        selection=[
            ...
            ("29", "NON-DOMICILED SALES THAT DO NOT QUALIFY AS EXPORTS"),
            ("30", "OPERATION SUBJECT TO DETRACTION"),
            ("33", "DETRACTION - CARGO TRANSPORTATION SERVICES"),
            ("34", "OPERATION SUBJECT TO PERCEPTION"),
            ("32", "DETRACTION - PASSENGER TRANSPORTATION SERVICES"),
            ("31", "DETRACTION - HYDROBIOLOGICAL RESOURCES"),
        ],
    ...
    activate_einvoice_ticket = fields.Boolean(compute="_compute_activate_einvoice_ticket")
    ...
    def _onchange_partner_id(self):
        ...
        if self.partner_id:
            if self.partner_id.l10n_pe_edi_retention_type_id:
                self.l10n_pe_edi_retention_type_id = self.partner_id.l10n_pe_edi_retention_type_id

    @api.onchange("l10n_pe_edi_detraction_type_id")
    def _onchange_l10n_pe_edi_detraction_type_id(self):
        company_id = self.env.company
        if not self.l10n_pe_edi_detraction_payment_type_id:
        ...
        if self.l10n_pe_edi_detraction_type_id and self.l10n_pe_edi_odoofact_operation_type != "4":
            self.l10n_pe_edi_odoofact_operation_type = "30"
        elif not self.l10n_pe_edi_detraction_type_id and self.l10n_pe_edi_odoofact_operation_type != "4":
            self.l10n_pe_edi_odoofact_operation_type = "1"
    ...
    def _compute_l10n_pe_edi_is_sale_credit(self):
        for move in self:
            if move.l10n_latam_document_type_id.code in ["01", "03"] or not move.l10n_latam_document_type_id:
                if any(
                    line.date_maturity
    ...
    def check_data_to_send_extend(self):
        ...
        if self.l10n_pe_edi_detraction_type_id:
            ...
            if self.l10n_pe_edi_odoofact_operation_type not in ["4","30", "31", "32", "33"]:
                raise UserError(
                    _(
    ...
    def _get_guides_values_generar_odoofact(self):
        ...
        for guide in self.l10n_pe_edi_picking_number_ids:
            guide_list.append(
                {
                    "guia_tipo": int(guide.type) if guide.type else "",
    ...
    def get_dues_values_ticket(self):
        dues = self.l10n_pe_edi_dues_ids
        data = []
        description = ''
        contador = ''
        fecha = ''
        monto = ''
        trj = ''
        if self.l10n_pe_edi_is_sale_credit == True:
            for d in dues:
                contador = str(d.dues_number)
                fecha = datetime.strptime(str(d.paid_date), "%Y-%m-%d").strftime("%Y-%m-%d")
                monto = str(abs(d.amount))
                description = 'CUOTA ' + contador + ' ' + fecha + ' ' + '(S/' + monto + ')'
    ...
    def action_print_einvoice_ticket(self):
        report_action = self.env.ref('l10n_pe_edi_odoofact.account_einvoice_report_ticket').report_action(self)
        return report_action
    
    def action_view_guides(self):
        picking_ids = self._get_picking_ids()
        return {
            'type': 'ir.actions.act_window',
            'name': _('View Guides'),
            'res_model': 'l10n.pe.edi.stock.picking.massive',
            'view_mode': 'form',
            'target': 'new',
            'context': {
                'default_invoice_id': self.id,
                'default_picking_ids': [(6, 0, picking_ids)],
            },
        }
